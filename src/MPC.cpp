...
public:
	...
	void operator()(ADvector& fg, const ADvector& vars) {
		...
		const int steering_jerk_weight = 300;
		// Minimize the value gap between sequential actuations.
		for (int t = 0; t < N - 2; t++) {
			fg[0] += steering_jerk_weight * CppAD::pow(vars[delta_start + t + 1] - vars[delta_start + t], 2);
			fg[0] += steering_jerk_weight * CppAD::pow(vars[a_start + t + 1] - vars[a_start + t], 2);
		}
